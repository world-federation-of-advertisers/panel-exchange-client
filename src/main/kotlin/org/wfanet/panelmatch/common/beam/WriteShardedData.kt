...
class WriteShardedData<T : Message>(
  ...
  override fun expand(input: PCollection<T>): WriteResult {
    ...
    val presentKeys = groupedData.keys("Present Keys").toListView("Present Keys View")

    val missingFiles: PCollection<KV<Int, Iterable<T>>> =
      input.pipeline.apply("Add Missing Files/Create", Create.of(true)).parDoWithSideInput(
          presentKeys
        ) { _, keys ->
