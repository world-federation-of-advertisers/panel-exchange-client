...
internal class WriteSignedFiles(private val fileSpec: String, private val privateKey: PrivateKey) :
  ...
  override fun expand(input: PCollection<ByteString>): WriteFilesResult<Void> {
    return input.apply(
      FileIO.write<ByteString>()
        ...
        .via(SignedFileSink(fileSpec, privateKey))
    )
  }
}

private class SignedFileSink(private val fileSpec: String, private val privateKey: PrivateKey) :
  ...
  override fun open(channel: WritableByteChannel) {
    ...
    signer = Signature.getInstance(privateKey.algorithm, jceProvider)
