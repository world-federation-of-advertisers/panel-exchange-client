...
object SignedFiles {
   ...
   * @param x509Certificate certificate to use to verify the files
   */
  fun read(
    fileSpec: String,
    x509Certificate: X509Certificate
  ): PTransform<PBegin, PCollection<ByteString>> {
    return ReadSignedFiles(fileSpec, x509Certificate)
  }

  /**
   * Writes ByteStrings to sharded files. Each file is prepended with a signature.
   *
   * TODO: load PrivateKey from KMS securely in each task.
   *
   * @param fileSpec URI specification of the form "protocol://some/path/to/files-*-of-00123".
  ...
  fun write(
    ...
    return WriteSignedFiles(fileSpec, privateKey)
  }
}
