...
class ReadSignedFiles(
  ...
  override fun expand(input: PBegin): PCollection<ByteString> {
    ...
    return files.apply(
        "ReadSignedFiles",
        ParDo.of(ReadFilesFn(fileSpec, fileCount, x509Certificate)).withSideInputs(fileCount)
      )
...
private class ReadFilesFn(
  ...
  fun processElement(@Element readableFile: ReadableFile, context: ProcessContext) {
    ...
    val fileUri = readableFile.metadata.resourceId().toString()
    require(fileUri.endsWith("-of-%05d".format(fileCount))) { "Unexpected file suffix: $fileUri" }

    Channels.newInputStream(readableFile.open()).use { stream ->
      val codedInput = CodedInputStream.newInstance(stream)

      val verifier = Signature.getInstance(x509Certificate.sigAlgName, jceProvider)
