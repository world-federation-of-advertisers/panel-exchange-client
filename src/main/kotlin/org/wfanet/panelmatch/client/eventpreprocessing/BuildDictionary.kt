...
class BuildDictionary(private val dictionaryBuilder: DictionaryBuilder) :
  PTransform<PCollection<KV<ByteString, ByteString>>, PCollection<Dictionary>>() {

  override fun expand(events: PCollection<KV<ByteString, ByteString>>): PCollection<Dictionary> {
    return events
      .values()
      .apply(
        "Rough Sample",
