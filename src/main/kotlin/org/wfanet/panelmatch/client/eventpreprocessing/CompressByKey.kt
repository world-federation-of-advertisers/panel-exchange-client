...
fun EventCompressorTrainer.compressByKey(
  ...
  val eventAggregatorView = trainedEventCompressor.map { it.compressor }.apply(View.asSingleton())

  val aggregatedEvents: PCollection<KV<ByteString, ByteString>> =
    events
      .groupByKey()
      .mapValues { combinedEvents { serializedEvents += it }.toByteString() }
      .parDoWithSideInput(eventAggregatorView) {
  ...
  return CompressedEvents(aggregatedEvents, dictionaryView)
}
