...
class CoroutineLauncher : JobLauncher {
    ...
    private fun executeStep(apiClient: ApiClient, exchangeStep: ExchangeStep, attempt: ExchangeStepAttempt.Key) = scope.launch {
        LOGGER.info("Executing ${exchangeStep.toString()} with attempt ${attempt.toString()}")
        val storage = InMemoryStorage()
        val output = ExchangeTaskMapper()
                .execute(
                    exchangeStep.step,
                    emptyMap<String, ByteString>(),
                    storage
                )
      //val request =
/*        FinishExchangeStepAttemptRequest.newBuilder()
    ...
    override suspend fun execute(apiClient: ApiClient, exchangeStep: ExchangeStep, attempt: ExchangeStepAttempt.Key) {
