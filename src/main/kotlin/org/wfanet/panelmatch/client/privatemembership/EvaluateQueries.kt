...
): PCollection<KV<ShardId, EncryptedQueryResults>> {
...
) : PTransform<PCollectionTuple, PCollection<KV<ShardId, EncryptedQueryResults>>>() {

  override fun expand(input: PCollectionTuple): PCollection<KV<ShardId, EncryptedQueryResults>> {
...
private class EvaluateQueriesForShardFn(
  ...
  private val queryEvaluator: QueryEvaluator,
  private val serializedPublicKey: PCollectionView<ByteString>
) : DoFn<KV<ShardId, CoGbkResult>, KV<ShardId, EncryptedQueryResults>>() {
  ...
  fun processElement(context: ProcessContext) {
    ...
    context.output(kvOf(context.element().key, encryptedQueryResults { this.results += results }))
  }
