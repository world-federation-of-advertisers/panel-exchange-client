  ...
  fun `executeQueries on multiple shards with multiple QueryBundles`() {
    ...
    assertThat(results.map { it.queryId to helper.decodeResultData(it) })
      .containsExactly(
        ...
        queryIdOf(502) to BucketContents.getDefaultInstance(),
        queryIdOf(503) to BucketContents.getDefaultInstance(),
        queryIdOf(504) to BucketContents.getDefaultInstance(),
        queryIdOf(505) to BucketContents.getDefaultInstance(),
        queryIdOf(506) to BucketContents.getDefaultInstance(),
