...
private const val BUCKETS_PER_SHARD_COUNT = 1 shl 19
...
fun main(args: Array<String>) {
  ...
  val privateMembershipKeys =
    pipeline.apply(
      ...
      Create.ofProvider(
        object : ValueProvider<PrivateMembershipKeys> {
  ...
  val rawQueries: PCollection<PanelistKeyAndJoinKey> =
    pipeline.apply("Create Queries", Create.of(0 until SHARD_COUNT)).parDo("Populate Queries") { i
      ->
      for (j in 0 until QUERIES_PER_SHARD_COUNT / 2) {
        yield(
  ...
  val queryEvaluator = JniQueryEvaluator(PRIVATE_MEMBERSHIP_PARAMETERS)
  ...
  val outputSchema =
    TableSchema()
      .setFields(
        listOf(
          TableFieldSchema().setName("QueryId").setType("INT32"),
          TableFieldSchema().setName("Result").setType("BYTES")
        )
      )

  results
    .mapWithSideInput(privateMembershipKeys.toSingletonView()) { encryptedQueryResult, keys ->
      val response =
      ...
      TableRow()
        .set("QueryId", encryptedQueryResult.queryId.id)
        .set("EncryptedResult", Base64.getEncoder().encode(result.toByteArray()))
    }
