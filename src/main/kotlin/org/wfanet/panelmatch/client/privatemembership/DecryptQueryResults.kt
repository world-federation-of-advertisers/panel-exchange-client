...
class DecryptQueryResults(
  ...
  override fun expand(input: PCollectionTuple): PCollection<KeyedDecryptedEventDataSet> {
    ...
    val paddingQueryResults = groupedDecryptedResults.filter { it.key.isPaddingQuery }
    val realQueryResults = groupedDecryptedResults.filter { !it.key.isPaddingQuery }

    val paddingQueryKeyedDecryptedEventDataSets =
      paddingQueryResults.map {
