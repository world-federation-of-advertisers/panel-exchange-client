 ...
 * @param batchLookupQueryEvaluator implementation of lower-level homomorphic operations
 */
class BatchLookupWorkflow(
  private val parameters: Parameters,
  private val batchLookupQueryEvaluator: BatchLookupQueryEvaluator
) : Serializable {
  ...
  fun batchLookup(
    ...
    return uncombinedResults
      .combinePerKey("Combine Subshard Results") {
        batchLookupQueryEvaluator.combineResults(it.asSequence())
      }
  ...
  internal fun querySubshards(
    ...
    return shardedDatabase.join(queriesByShard, name = "Join Database and queryMetadata") {
      ...
      for (shard in shards) {
        val results = batchLookupQueryEvaluator.executeQueries(listOf(shard), queriesList)
