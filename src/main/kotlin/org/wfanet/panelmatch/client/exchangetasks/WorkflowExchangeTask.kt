...
class WorkflowExchangeTask(private val workflow: Workflow) : ExchangeTask {
  ...
  interface Workflow {
     ...
     * @return a map from output name to a URI pattern where it was written to
     */
    suspend fun execute(inputUris: Map<String, String>): Map<String, String>
  }
  ...
  override suspend fun execute(input: Map<String, VerifiedBlob>): Map<String, Flow<ByteString>> {
    ...
    val outputUris = workflow.execute(inputUris)
    return outputUris.mapValues { makeOutputBlob(it.value) }
  }
}
