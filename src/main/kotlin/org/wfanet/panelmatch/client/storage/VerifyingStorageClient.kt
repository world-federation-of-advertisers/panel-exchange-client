...
class VerifyingStorageClient(
  ...
  suspend fun getBlob(blobKey: String): VerifiedBlob {
    ...
    return VerifiedBlob(SignedBlob(sourceBlob, namedSignature.signature))
  }

  /** [Blob] wrapper that ensures the blob's signature is verified when read. */
  inner class VerifiedBlob(private val sourceBlob: SignedBlob) {
    val size: Long
      get() = sourceBlob.size
    ...
    fun read(): Flow<ByteString> = sourceBlob.readVerifying(this@VerifyingStorageClient.x509)
