...
absl::StatusOr<DecryptEventDataResponse> DecryptEventData(
  ...
  for (const std::string& encrypted_event : request.encrypted_event_data().ciphertexts()) {
    DecryptedEventData *decrypted_event_data = response.add_decrypted_event_data();;
    ASSIGN_OR_RETURN(
