...
auto* const global_key_loader_registry = new KeyLoaderRegistry;
}  // namespace
...
absl::StatusOr<::crypto::tink::util::SecretData> ApplyGlobalKeyLoaderIfSet(
    absl::string_view key_name) {
  if (KeyLoader* key_loader = GetGlobalKeyLoader(); key_loader != nullptr) {
    return key_loader->LoadKey(key_name);
  }
  return SecretDataFromStringView(key_name);
}
