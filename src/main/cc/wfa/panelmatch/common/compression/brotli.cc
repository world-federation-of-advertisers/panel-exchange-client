...
class Brotli : public Compressor {
 public:
  ...
  absl::StatusOr<std::string> Compress(
    ...
    std::string result;
    BrotliWriter brotli_writer(StringWriter<std::string *>(&result), writer_options_);
  ...
  absl::StatusOr<std::string> Decompress(
    ...
    std::string result;
    BrotliReader brotli_reader(StringReader<absl::string_view *>(&compressed_data), reader_options_);
