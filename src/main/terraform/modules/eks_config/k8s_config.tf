...
resource "null_resource" "configure_metrics_and_logging" {
...
sed -i 's|{{cluster-name}}|${var.cluster_name}"|' cwagent-configmap.yaml
kubectl apply -f cwagent-configmap.yaml
kubectl apply -f https://raw.githubusercontent.com/aws-samples/amazon-cloudwatch-container-insights/latest/k8s-deployment-manifest-templates/deployment-mode/daemonset/container-insights-monitoring/cwagent/cwagent-daemonset.yaml
    EOF
  }

  # setup prometheus (monitoring K8S metrics)
  provisioner "local-exec" {
    command = <<EOF
# https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/ContainerInsights-Prometheus-Setup.html
eksctl utils associate-iam-oidc-provider --region=${data.aws_region.current.name} --cluster=${var.cluster_name} --approve
...
project_name=$(echo $var.cluster_name | sed -r 's/^(.*)-Cluster$/\1/')
kubectl annotate serviceaccounts fluent-bit -n amazon-cloudwatch "eks.amazonaws.com/role-arn=arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${project_name}"
  }
}
