...
private val ASYMMETRIC_KEYS =
  AsymmetricKeys("public-key".toByteString(), "private-key".toByteString())
...
private val PRIVATE_MEMBERSHIP_SERIALIZED_PARAMETERS = "some serialized parameters".toByteString()
      ...
      assertThat(deserializedResults)
        .containsExactly(
          "some-hashed-joinkey-1" to
            listOf(
              "some-lookup-key-1".toByteString(),
              HKDF_PEPPER,
          ...
          "some-hashed-joinkey-1" to listOf("payload-1".toByteString()),
          "some-hashed-joinkey-2" to
            listOf(
              "some-lookup-key-2".toByteString(),
              HKDF_PEPPER,
              PRIVATE_MEMBERSHIP_SERIALIZED_PARAMETERS,
              COMPRESSION_PARAMETERS.toByteString(),
              ASYMMETRIC_KEYS.serializedPublicKey,
          ...
          "some-hashed-joinkey-2" to listOf("payload-2".toByteString()),
          "some-hashed-joinkey-3" to
            listOf(
              "some-lookup-key-3".toByteString(),
              HKDF_PEPPER,
              PRIVATE_MEMBERSHIP_SERIALIZED_PARAMETERS,
              COMPRESSION_PARAMETERS.toByteString(),
              ASYMMETRIC_KEYS.serializedPublicKey,
          ...
          "some-hashed-joinkey-3" to listOf("payload-3".toByteString()),
        )
...
private fun encryptedQueryResultOf(queryId: Int, payload: String): EncryptedQueryResult {
  return encryptedQueryResult {
    ...
    serializedEncryptedQueryResult = payload.toByteString()
  }
}
