...
class PipelineTest : BeamTestBase() {
  private fun makeWorkflow(
    ...
    return BatchLookupWorkflow(parameters, FakeBatchLookupQueryEvaluator)
  }
  ...
  fun querySubshards() {
    ...
    val queries =
      pcollectionOf(
        ...
        shardedQueryBundleOf(shard = 5, data = "bundle-has-no-query", queryIds = emptyList()),
...
private fun unaggregatedResultOf(
  ...
  return kvOf(kvOf(ShardId(shard), result.queryMetadata.queryId), result)
}

private fun bucketOf(id: Int, contents: String): Bucket {
  return Bucket(BucketId(id), contents.toByteString())
}

private fun queryMetadataOf(queryId: Int): QueryMetadata {
  return QueryMetadata(QueryId(queryId), "query-metadata-for-$queryId".toByteString())
}

private fun queryBundleOf(shard: Int, data: String, queryIds: List<Int>): QueryBundle {
  return QueryBundle(ShardId(shard), queryIds.map(::queryMetadataOf), data.toByteString())
}
...
private fun makeFakeQueryResult(query: Int, bundleData: String, buckets: Iterable<String>): Result {
  ...
  val queryMetadata = queryMetadataOf(query)
  val resultString =
    "QueryResult(queryMetadata=$queryMetadata, bundleData=$bundleData, buckets=[$bucketsString])"
  return Result(queryMetadata, resultString.toByteString())
}
...
/**
 * This [BatchLookupQueryEvaluator] implementation assuming ByteArrays are UTF8 strings of plaintext
 * numbers.
 */
private object FakeBatchLookupQueryEvaluator : BatchLookupQueryEvaluator {
  ...
  override fun executeQueries(
    ...
    for (shard in shards) {
      ...
      for (bundle in queryBundles) {
        ...
        for (query in bundle.queryMetadata) {
          add(makeFakeQueryResult(query.queryId.id, bundleData, buckets))
        }
      }
    }
  }
  ...
  override fun combineResults(results: Sequence<Result>): Result {
    ...
    return Result(resultsList[0].queryMetadata, combined.toByteString())
  }
}
