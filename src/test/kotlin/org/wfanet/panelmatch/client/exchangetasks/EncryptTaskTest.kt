...
class EncryptTaskTest {
  ...
  @Test
  fun `encryptTask with crypto error`() {
  ...
  @Test
  fun `encryptTask with missing inputs`() {
    runBlocking {
      assertFailsWith(IllegalArgumentException::class) {
        EncryptTask(commutativeEncryption)
          .execute(
      ...
      assertFailsWith(IllegalArgumentException::class) {
        EncryptTask(commutativeEncryption).execute(mapOf("encryption-key" to KEY), fakeSendDebugLog)
      }
    }
