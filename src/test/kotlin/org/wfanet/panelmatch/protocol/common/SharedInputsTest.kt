...
class SharedInputsTest {
  ...
  @Test
  fun `check SharedInputs serializer and parser`() {
    ...
    val serialized: ByteString = makeSerializedSharedInputs(someListOfByteString)
    val deserialized: List<ByteString> = parseSerializedSharedInputs(serialized)

    assertThat(deserialized).isEqualTo(someListOfByteString)
  }
}
