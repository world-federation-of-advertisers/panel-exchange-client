...
class SignedFilesTest : BeamTestBase() {
  ...
  private val inputValues = listOf("ABC", "DEF", "HIJ")
  ...
  private val fileSpec by lazy { baseDirectory.resolve("foo-*-of-00012").toUri().toASCIIString() }
  ...
  fun roundTrip() {
    ...
    inputs.apply(SignedFiles.write(fileSpec, privateKey))
    pipeline.run()

    assertThat(baseDirectory.toFile().listFiles()).hasLength(12)
